...
import 'package:FeSekka/ui/requestScreens/user_request_list_cell.dart';
...
class UserRequestListScreen extends StatefulWidget {
  const UserRequestListScreen({key});

  @override
  State<UserRequestListScreen> createState() => _AddNewRequestScreenState();
}

class _AddNewRequestScreenState extends State<UserRequestListScreen> {
  ...
  getData() async {
  data = await requestServices().getUserRequests();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isLoading?Loader():data.length==0?Center(
        ...
        return CarListWidget(carData: data[index],);
            }
            ),
      ),
    );
